...
>
								...
								<c:forEach items="${list_zz_type_analysis}"  var="zz_analysis"  >
									...
									<tr >
										...
										-->
										<td><a href='#'onclick="javascript:clickone('${zz_analysis.ac}','${zz_analysis.ac_type}');">${zz_analysis.ac_min}</td>
...
$(document).ready(function(){ 
	$('#zhangzu_ac').change(function(){ 
		...
		window.location.href = "<%=request.getContextPath() %>/zhangzu/to_zhangzu_analysis_2020.do?zhangzu_ac="+p1;
	}) 
}) 
 function clickone(zz_analysis_ac,zz_analysis_ac_type) {
	 zz_analysis_ac_type = encodeURIComponent(zz_analysis_ac_type);
	 //alert(zz_analysis_ac_type);
	 window.location.href = '${pageContext.request.contextPath}/zhangzu/to_index_zhangzu.do?AC='+ zz_analysis_ac +'&IO=MIN&AC_TYPE=' + zz_analysis_ac_type ;
 }
</script>
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['Month', '支出', '收入', '余额', '买货'],
		<c:forEach items="${list_2020_zz_analysis}"  var="zz_analysis"  varStatus="status">
		...
		function selectHandler() {
		  ...
		  for (var i = 0; i < selection.length; i++) {
			...
			if (item.row != null && item.column != null) {
			  ...
			  //ZHICHU
			  if (item.column == 1) {
				window.location.href = "/Blog/zhangzu/to_zhangzu_analysis_2020.do?zhangzu_ac="+str_ac;
			  }
