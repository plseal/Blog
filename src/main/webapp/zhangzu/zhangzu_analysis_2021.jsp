...
$(document).ready(function(){ 
	$('#zhangzu_ac').change(function(){ 
		...
		window.location.href = "<%=request.getContextPath() %>/zhangzu/to_zhangzu_analysis_2021.do?zhangzu_ac="+p1;
	}) 
}) 
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['Month', '支出', '收入', '余额', '买货'],
		<c:forEach items="${list_2021_zz_analysis}"  var="zz_analysis"  varStatus="status">
		...
		function selectHandler() {
		  ...
		  for (var i = 0; i < selection.length; i++) {
			...
			if (item.row != null && item.column != null) {
			  ...
			  //ZHICHU
			  if (item.column == 1) {
				window.location.href = "/Blog/zhangzu/to_zhangzu_analysis_2021.do?zhangzu_ac="+str_ac;
			  }
